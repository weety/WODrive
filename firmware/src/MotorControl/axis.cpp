...
void Axis::start_thread() {
    ...
    rt_completion_init(&thread_id_done);
    thread_id_ = rt_thread_create("state_machine_loop", run_state_machine_loop_wrapper, this, stack_size_, hw_config_.thread_priority, 10);
    rt_thread_startup(thread_id_);
...
void Axis::signal_current_meas() {
    if (thread_id_valid_)
        ...
        rt_completion_done(&thread_id_done);
}
...
bool Axis::wait_for_current_meas() {
    ...
    return RT_EOK == rt_completion_wait(&thread_id_done, rt_tick_from_millisecond(PH_CURRENT_MEAS_TIMEOUT));
}

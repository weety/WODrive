...
void ODriveCAN::can_server_thread() {
    for (;;) {
        ...
        if (status == HAL_CAN_ERROR_NONE) {
            ...
            rt_sem_take(sem_can, rt_tick_from_millisecond(10));
            while (available()) {
                read(rxmsg);
...
bool ODriveCAN::start_can_server() {
    ...
    thread_id_ = rt_thread_create("can_server", can_server_thread_wrapper, this, stack_size_, COMM_CAN_THREAD_PRIO, 10);

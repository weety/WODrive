...
extern "C" {
extern rt_sem_t sem_usb_tx;
extern rt_sem_t sem_usb_rx;
extern USBD_HandleTypeDef hUsbDeviceFS;
}

class USBSender : public PacketSink {
public:
    USBSender(uint8_t endpoint_pair, const rt_sem_t& sem_usb_tx)
    ...
    int process_packet(const uint8_t* buffer, size_t length) {
        ...
        if (rt_sem_take(sem_usb_tx_, rt_tick_from_millisecond(PROTOCOL_SERVER_TIMEOUT_MS)) != RT_EOK) {
...
static void usb_server_thread(void * ctx) {
    ...
    for (;;) {
        ...
        //if (sem_stat == osOK) {
        rt_err_t sem_stat = rt_sem_take(sem_usb_rx, RT_WAITING_FOREVER);
        if (sem_stat == RT_EOK) {
            usb_stats_.rx_cnt++;
